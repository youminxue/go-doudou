...
require (
	...
	github.com/youminxue/v2 ` + version.Release + `
)`
...
RUN export GDD_VER=$(go list -mod=vendor -m -f '{{` + "`" + `{{` + "`" + `}} .Version {{` + "`" + `}}` + "`" + `}}' github.com/youminxue/v2) && \
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -v -ldflags="-s -w -X 'github.com/youminxue/v2/framework/buildinfo.BuildUser=$HOST_USER' -X 'github.com/youminxue/v2/framework/buildinfo.BuildTime=$(date)' -X 'github.com/youminxue/v2/framework/buildinfo.GddVer=$GDD_VER'" -mod vendor -o api cmd/main.go && \
